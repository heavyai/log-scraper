...
fn sql_execute_nonce() {
    let rec = pln(r#"
2020-07-01T00:00:00.000106 I 15 DBHandler.cpp:1058 stdlog sql_execute 1 1425 omnisci admin 455-NzOR {"query_str","client","nonce","execution_time_ms","total_time_ms"} {"SELECT count(*) from omnisci_states;","tcp:172.17.0.1:52290","100/13","1418","1425"}
"#);
    assert_eq!(rec.event.unwrap(), "sql_execute");
    assert_eq!(rec.operation.unwrap(), "SELECT");
    assert_eq!(rec.query.unwrap(), "SELECT count(*) from omnisci_states;");
...
"#.trim());
    assert_eq!(rec.dashboardid.unwrap(), "15");
}
